...
| O-17 | `storefrontApi()` (লেআউট) সার্ভারের `m` মেসেজ নিজেই `storeToast()` দিয়ে দেখায় — কল-সাইট কোড (যেমন PDP-র "Add to Cart") একই সফল/ব্যর্থ রেসপন্সে আবার `storeToast()` কল করবে না | `product.php`-তে ডাবল করে কল হওয়ায় "Added to cart." দুইবার দেখা যাচ্ছিল (ব্যবহারকারীর রিপোর্ট, ২০২৬-০৯-১১); ক্লায়েন্ট-সাইড প্রি-ভ্যালিডেশন এরর (যেমন "Please select all options.") ভিন্ন — সার্ভার কল-ই হয়নি, তাই ওগুলো ডুপ্লিকেট না |
| O-18 | PDP মেইন গ্যালারি ছবিতে হোভার করলে গোল ম্যাগনিফায়ার লেন্স (vanilla JS, কার্সর-ফলো, কোনো লাইব্রেরি না) | রেফারেন্স সাইটগুলোর মতো প্রোডাক্ট ছবি জুম করে কাপড়ের টেক্সচার/স্টিচিং দেখার সুবিধা; প্রজেক্টে বাইরের JS ফ্রেমওয়ার্ক/লাইব্রেরি নাই (raw PHP কনভেনশন), তাই CSS `background-position` ক্যালকুলেশন দিয়েই বানানো — মোবাইলে (hover নাই) লেন্স `display:none` |
